(function(){
  ...
  function logoUrl(name){const d=institutionDomain(name);return d?`https://www.google.com/s2/favicons?domain_url=https://${encodeURIComponent(d)}&sz=128`:""}
  ...
  function openProfile(n){els.profileName.textContent=n.name;els.profileAffiliation.textContent=n.affiliation||"Academic lineage";const logoInstitution=institutionDomain(n.affiliation)?n.affiliation:n.degreeInstitution,logo=logoUrl(logoInstitution);els.profileLogo.innerHTML=logo?`<img src="${logo}" alt="${esc(logoInstitution)} institution mark" onerror="this.remove();this.parentElement.textContent='${esc(initials(logoInstitution||n.name))}'">`:esc(initials(logoInstitution||n.name));els.profileDetails.innerHTML=`<div><dt>Academic path</dt><dd>${esc(n.details)}</dd></div><div><dt>Genealogy branch</dt><dd>${esc(n.section)}</dd></div>${n.parentId?`<div><dt>Relationship</dt><dd>${n.relation==="mphil"?"M.Phil.":n.relation==="postdoc"?"Postdoctoral mentorship":"Ph.D."}</dd></div>`:""}`;els.profileLink.href=n.url;els.suggest.href=issueUrl(n);els.panel.classList.add("open")}
  ...
  function setZoom(next){scale=Math.max(.38,Math.min(1.2,next));els.canvas.style.transform=`scale(${scale})`;els.zoom.textContent=`${Math.round(scale*100)}%`}
  function bind(){
